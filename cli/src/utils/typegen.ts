...
export const TOP_LEVEL_NAMESPACE = '__TOP_LEVEL_MODULE__'
export const DEFAULT_TYPE_DEF_HEADER = `/* auto-generated by NAPI-RS (CURSOR FORK 1.0) */
/* eslint-disable */
`
...
function processOneTypedef(defs: TypeDefLine[], constEnum: boolean): { declaration: string, exports: string[] } {
...
export async function processTypeDefModules(
  ...
  const sortedNamespaces = sortBy(Array.from(groupedDefs), [([namespace]) => namespace]);
  const declarations: Map<string, { declaration: string, exports: string[] }> = new Map()

  for (const [namespace, defs] of sortedNamespaces) {
    if (declarations.has(namespace)) {
      throw new Error(`Duplicate namespace: ${namespace}`)
    }

    const { declaration, exports } = processOneTypedef(defs, constEnum)
    declarations.set(namespace, { declaration, exports })
  }
    ...
    sortBy(Array.from(groupedDefs), [([namespace]) => namespace])
      .map(([namespace, defs]) => {
        if (namespace === TOP_LEVEL_NAMESPACE) {
          const { declaration, exports: addExports } = processOneTypedef(defs, constEnum)
          exports.push(...addExports)
          return declaration
        } else {
